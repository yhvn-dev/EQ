      ...
      <main className="full  shadow-lg column-t row-start-3 row-end-3 col-start-1  lg:row-start-1 lg:row-end-span-full lg:col-start-2 lg:col-span-full overflow-y-hidden  lg:overflow-y-auto" >
        ...
        <nav className="w-full center h-[30%] p-2 ">
          ...
          <div className='full flex items-center justify-start flex-row-reverse'>
            <button  onClick={handleOpenLogin} className='shadog-lg px-4 py-2  cursor-pointer rounded-xl center hover:bg-[var(--moon-phases-e)] bg-[var(--moon-phases-d)] stroke="#fff"'><Plus size={18}  fill='#fff' /></button>
          </div>
        </nav>
           ...
           <tbody className='w-full overflow-hidden  lg:overflow-y-auto'>
              {eq.length > 0 && eq.map((e)=>{
                return (
                  <>
                  <tr className='td-tr'>
                    <td className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.dateTime}</td>
                    <td className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.depth}</td>
                    <a href={e.detailLink} className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.detailLink}</a>
                    <td className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.latitude}</td>
                    <td className="text-sm px-4 py-1 sm:text-wrap wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.location}</td>
                    <td className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.longitude}</td>
                    <td className="text-sm px-4 py-1 sm:text-wrap sm:wrap-break-word break-normal lg:text-wrap lg:wrap-break-word">{e.magnitude}</td>
                  </tr>
                  </>
                )
              })}
